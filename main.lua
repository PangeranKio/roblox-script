-- [[ VOIDHUB CUSTOM UI - ULTRA SUPREME EDITION v4.0 ]] --
-- Created by Kio (Advanced Egg Tracker UI, Auto Steal, Treadmill, & Config System)
...
-- ==========================================
local ConfigFileName = "VoidHub_Config_Kio.json"
local SavedConfig = {
    AutoStealActive = false,
    StealRarity = "Divine",
    StealLocation = "Demon/Angel",
    AutoTreadmill = true,
    PlayerESP = false,
    EggESPUI = false,
    WalkSpeedVal = 24
...
-- ==========================================
-- 1. FLOATING BUTTON: "Void"
-- ==========================================
local OpenBtn = Instance.new("TextButton")
OpenBtn.Name = "OpenButton"
OpenBtn.Size = UDim2.new(0, 68, 0, 36)
OpenBtn.Position = UDim2.new(0.08, 0, 0.22, 0)
OpenBtn.BackgroundColor3 = Color3.fromRGB(18, 10, 30)
OpenBtn.BackgroundTransparency = 0.15
OpenBtn.Text = "Void"
OpenBtn.TextColor3 = Color3.fromRGB(230, 170, 255)
OpenBtn.TextSize = 13
...
OpenGlow.Color = Color3.fromRGB(190, 110, 255)
OpenGlow.Transparency = 0.3
OpenGlow.Thickness = 1.5
OpenGlow.Parent = OpenBtn

MakeDraggable(OpenBtn, OpenBtn)

-- ==========================================
-- 2. LOADING SCREEN LUXURY
-- ==========================================
...
LoadGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, Color3.fromRGB(60, 20, 95)),
...
LoadTitle.Text = "VOIDHUB <font color=\"#D480FF\">SUPREME</font>"
...
LoadStatus.Text = "Loading Config & Secure Modules..."
...
-- ==========================================
local TargetSize = UDim2.new(0, 520, 0, 320)
...
GlassGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, Color3.fromRGB(45, 16, 75)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(14, 7, 24)),
...
Title.Text = "VoidHub <font color=\"#C080FF\">Supreme Engine</font>"
Title.RichText = true
...
local function CreateToggle(parent, titleText, defaultState, callback)
    ...
    switch.BackgroundColor3 = defaultState and Color3.fromRGB(140, 70, 240) or Color3.fromRGB(40, 22, 60)
    switch.Text = ""
    switch.Parent = frame
...
-- ==========================================
-- TAB 1: STEAL AN EGG (EGG ESP & TRACKER UI)
-- ==========================================

-- Container / List UI untuk menampilkan telur berdasarkan kelangkaan, ukuran, dan harga dari atas ke bawah
local EggTrackerContainer = Instance.new("ScrollingFrame")
EggTrackerContainer.Size = UDim2.new(1, 0, 0, 180)
EggTrackerContainer.BackgroundColor3 = Color3.fromRGB(20, 10, 32)
EggTrackerContainer.BackgroundTransparency = 0.5
...
ETCCorner.CornerRadius = UDim.new(0, 10)
...
ETCLayout.Padding = UDim.new(0, 4)
ETCLayout.Parent = EggTrackerContainer

-- Fungsi pembantu mendeteksi kelangkaan teks
local function GetRarityWeight(name)
    local l = name:lower()
    if l:find("divine") then return 6
    elseif l:find("mythic") then return 5
    elseif l:find("legendary") then return 4
    elseif l:find("epic") then return 3
    elseif l:find("rare") then return 2
    else return 1 end
end

-- Fungsi memperbarui Tracker UI & Egg ESP
local function RefreshEggTracker()
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        ...
        if (nameLower:find("egg") or nameLower:find("telur")) and not nameLower:find("player") then
            local targetPart = obj:IsA("BasePart") and obj or (obj:IsA("Model") and (obj.PrimaryPart or obj:FindFirstChildWhichIsA("BasePart")))
            if targetPart then
                local weight = GetRarityWeight(obj.Name)
                local sizeVal = targetPart.Size.Magnitude
                table.insert(eggList, {Object = obj, Part = targetPart, Name = obj.Name, Weight = weight, Size = sizeVal})
            end
        end
    end
    
    -- Urutkan dari kelangkaan, ukuran, & harga tertinggi ke terbawah
    ...
    for _, item in ipairs(eggList) do
        -- Update ESP Highlight (Hanya di objek telur, bukan item lain)
        ...
        -- Buat baris informasi di UI Tracker
        local row = Instance.new("Frame")
        row.Size = UDim2.new(1, -6, 0, 32)
        row.BackgroundColor3 = Color3.fromRGB(35, 18, 55)
...
task.spawn(function()
    pcall(function()
        ...
        task.wait(0.3)
        LoadStatus.Text = "Rendering Supreme Glass UI..."
        task.wait(0.3)
    end)
